...
/**
 * V12 runtime core policy: core packages are bundled into the APK at build time.
...
object BundledModelStores {
    ...
    fun laneCore(context: Context): Result<File> {
        ...
        if (!copyAsset(context, LANE_CORE_NAME, out) || !isValidLaneCore(out)) {
            out.delete()
            return Result.failure(IllegalStateException("APK thiếu hoặc sai mô-đun Lane Core. Hãy rebuild V12 để đóng model vào APK."))
        }
    ...
    fun roadUsers(context: Context): Result<File> {
        ...
        if (!copyAsset(context, ROAD_CORE_NAME, out) || !isValidRoadCore(out)) {
            out.delete()
            return Result.failure(IllegalStateException("APK thiếu hoặc sai mô-đun Road Core. Hãy rebuild V12 để đóng model vào APK."))
        }
        return Result.success(out)
    }

    ...
    doLast {
        ...
        fun download(url: String, target: File) {
            ...
            val connection = (URI(url).toURL().openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("User-Agent", "TRUNGKIEN-V12-Core-Build/12.0.0")
        ...
        if (!visionValid(visionOut)) {
            ...
            if (visionValid(manualVision)) {
                ...
                logger.lifecycle("TRUNGKIEN V12: preparing Road Core package...")
                download(
        ...
        logger.lifecycle("TRUNGKIEN V12: Road Core ready (${visionOut.length()} bytes).")
        ...
        if (!laneValid(laneOut)) {
            ...
            if (laneValid(manualLane)) {
                ...
                logger.lifecycle("TRUNGKIEN V12: preparing Lane Core package...")
                download(
        ...
        logger.lifecycle("TRUNGKIEN V12: Lane Core ready (${laneOut.length() / 1024 / 1024} MiB).")
        logger.lifecycle("TRUNGKIEN V12: core packages will be bundled into the APK; runtime download is disabled.")
    }
}

...
WORK="$HOME/.trungkien_v14_deploy"
...
say "TRUNGKIEN V14 CENTER-FIRST — ONE TAP DEPLOY + BUILD"
...
is_v14_project() {
  ...
  [ -d "$d/app" ] && [ -f "$d/settings.gradle.kts" ] &&
    grep -q 'versionName = "14.0.0"' "$d/app/build.gradle.kts" 2>/dev/null
}

for d in "$DOWNLOAD/TRUNGKIEN_V14.0.0_CENTER_FIRST_ADAS" "$DOWNLOAD/V14" "$DOWNLOAD/TRUNGKIEN_V14"; do
  if is_v14_project "$d"; then SRC="$d"; break; fi
done

if [ -z "$SRC" ]; then
  ZIP="$(find "$DOWNLOAD" -maxdepth 1 -type f \( -iname 'TRUNGKIEN_V14*.zip' -o -iname 'V14*.zip' \) -printf '%T@ %p\n' 2>/dev/null | sort -nr | cut -d' ' -f2- | head -n 1 || true)"
  [ -n "$ZIP" ] || die "Không thấy ZIP V14 trong Download. Hãy tải BUILD READY trước."
  ...
  [ -n "$PROJECT_GRADLE" ] || die "ZIP V14 không có settings.gradle.kts."
  SRC="$(dirname "$PROJECT_GRADLE")"
fi

is_v14_project "$SRC" || die "Source tìm thấy không phải V14.0.0."
[ -f "$SRC/.github/workflows/android.yml" ] || die "Source V14 thiếu workflow Android."
ok "Source V14: $SRC"
...
if ! git ls-remote --exit-code --heads origin pre-v14-stable >/dev/null 2>&1; then
  git branch pre-v14-stable HEAD
  git push origin pre-v14-stable
  ok "Đã tạo rollback pre-v14-stable."
fi
...
say "Đưa V14 CENTER-FIRST vào main..."
...
for p in app gradle .github gradlew build.gradle.kts settings.gradle.kts; do
  [ -e "$p" ] || die "Thiếu $p sau khi copy V14."
done

grep -q 'versionName = "14.0.0"' app/build.gradle.kts || die "Version source không đúng 14.0.0."
...
if git diff --cached --quiet; then
  CHANGED=0
  warn "Repo đã có đúng source V14; sẽ chạy workflow thủ công."
else
  git commit -m "TRUNGKIEN V14.0.0 CENTER-FIRST near-lane lead-priority cut-in-noise reduction"
  git push origin main
  ok "Đã push V14 CENTER-FIRST lên GitHub."
fi
...
say "GITHUB ĐANG BUILD V14 — RUN $RUN_ID"
if gh run watch "$RUN_ID" --repo "$REPO" --exit-status; then
  ok "Build V14 SUCCESS."
else
  die "Build V14 FAILURE. Gửi Run ID $RUN_ID cho ChatGPT để đọc log và sửa."
fi

APK_DIR="$DOWNLOAD/V14_APK"
rm -rf "$APK_DIR"
mkdir -p "$APK_DIR"

say "Tải APK về Download/V14_APK..."
SUCCESS=0
for n in 1 2 3; do
...
if [ "$SUCCESS" -eq 1 ]; then
  ok "HOÀN TẤT — APK nằm trong Download/V14_APK"
